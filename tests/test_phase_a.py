...
def test_karya_lifecycle(runner, temp_repo):
    with runner.isolated_filesystem(temp_repo):
        ...
        assert Path(".karya/tickets/backlog").exists()
...
def test_karya_adr_lifecycle(runner, temp_repo):
    with runner.isolated_filesystem(temp_repo):
...
def test_karya_block(runner, temp_repo):
    with runner.isolated_filesystem(temp_repo):
        runner.invoke(cli, ["init"])
