...
from karya.core.models import Priority, TicketStatus
from karya.exceptions import IncompleteAcceptanceCriteria, InvalidTransitionError
from karya.services.ticket_service import TicketService


def test_create_and_get(karya_root: Path) -> None:
    service = TicketService(karya_root)
...
def test_transition_moves_ticket(karya_root: Path) -> None:
    service = TicketService(karya_root)
...
def test_transition_done_requires_acceptance(karya_root: Path) -> None:
    service = TicketService(karya_root)
...
def test_log_appends_execution_entry(karya_root: Path) -> None:
    service = TicketService(karya_root)
...
def test_get_next_respects_priority_and_dependencies(karya_root: Path) -> None:
    service = TicketService(karya_root)
    ...
    next_ticket = service.get_next("agent")
    ...
    dependent.dependencies = [blocker.id]
    service.update(dependent.id, {"dependencies": [blocker.id]})
