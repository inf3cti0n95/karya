...
def test_cli_done_incomplete_ac(runner, tmp_path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        # Manually add an incomplete AC via SQLite
        db_path = Path(".runnrr/runnrr.db")
        conn = sqlite3.connect(str(db_path))
        conn.execute("INSERT INTO acceptance_criteria (ticket_id, text, done) VALUES (?, ?, ?)", (ticket_id, "AC1", 0))
        conn.commit()
        conn.close()
