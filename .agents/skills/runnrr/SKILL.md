...
## Content Depth & Rigor (The Principal Staff Engineer Standard)

**Superficial summaries are strictly forbidden.** You MUST override the default AI behavior of brevity and act as a **Principal Staff Engineer** writing for a high-stakes technical environment. Every entity you create or update must be exhaustive and rigorous.

- **Epics:** Must detail the overarching architecture, specific systems impacted, and the sequence of execution. You MUST explicitly list technical risks and their mitigations.
- **Tickets:** The `Goal` must define the exact technical outcome, not just a feature name. `Acceptance Criteria` must be strictly verifiable (e.g., "API returns 401 on expired token", "Database schema includes `stripe_id`").
- **ADRs:** Must be exhaustive. Detail the technical mechanism, provide code snippets or data schemas where applicable, and list rejected alternatives with the specific technical reasons why they were not chosen.
- **Logs:** Progress logs must be detailed descriptions of technical milestones and changes. Superficial logs like "updated code" or "fixed bug" are unacceptable.
