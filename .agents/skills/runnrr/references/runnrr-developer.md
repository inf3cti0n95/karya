...
5.  **Execute**: Write code. After each meaningful step, log your progress: `runnrr log <TICKET-ID> "<detailed technical description>"`.
    - **Rigor**: Progress logs MUST be technical and descriptive. Superficial logs like "fixed bug" or "updated code" are strictly forbidden. You MUST describe the specific technical change and its impact.
6.  **Progress**: To mark tasks and acceptance criteria as done, use the `runnrr update` command.
    -   `runnrr update <TICKET-ID> --check-task <index>`
    -   `runnrr update <TICKET-ID> --check-ac <index>`
    - **Verification**: You MUST empirically verify every Acceptance Criterion before checking it off. If a test is required, run the test and confirm the result.
