...
### 1. Requirement Breakdown (The Principal Standard)
When given a user requirement:
1.  **Search**: `runnrr search "<keywords>"` — check for existing or overlapping work.
2.  **Epics**: Create an Epic for any multi-ticket initiative: `runnrr epic create "<Title>"`. 
    - **Rigor**: Detailed architecture, system impact, and risks MUST be in the goal/notes.
...
6.  **Refine**: Use `runnrr update <TICKET-ID>` to define the `Goal`, `Tasks`, and `Acceptance Criteria`.
    - **Depth**: The `Goal` must explicitly state the technical outcome and expected behavior.
    - **Verifiability**: Every AC must be binary (either it passed or it didn't). Use specific constraints (e.g., "Returns 201 Created", "Schema updated with X field").

### 2. Architecture Decisions (The Exhaustive Protocol)
When a significant technical decision arises:
1.  **Draft**: `runnrr adr create "Title" --context "..." --decision "..."`.
2.  **Exhaustion**: The `decision` MUST detail the exact implementation mechanism. You MUST include code snippets or data schemas if applicable.
3.  **Alternatives**: The `alternatives` section MUST list at least two other approaches and the specific technical reasons they were rejected.
4.  **Accept**: Once finalized and reviewed, run `runnrr adr accept <ID>`.
