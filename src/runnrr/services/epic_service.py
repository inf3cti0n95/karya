...
from runnrr.core.parser import parse_epic, serialize_epic, parse_ticket
from runnrr.git.integration import GitIntegration
from runnrr.core.filesystem import TICKET_DIRS
...
class EpicService:
	...
	def create(
		...
		self._git.commit(f"runnrr: [{epic_id}] created ({actor or 'system'})")
		return epic
	...
	def update(self, epic_id: str, updates: dict, actor: str | None = None) -> Epic:
		...
		self._save(epic)
		self._git.commit(f"runnrr: [{epic_id}] updated ({actor or 'system'})")
		return epic
