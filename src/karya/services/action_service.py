...
class ActionService:
    ...
    def valid_actions(self, ticket_id: str) -> List[Dict[str, Any]]:
        ...
        start_action = {
            "action": "start_ticket",
            "command": f"karya start {ticket_id}",
        ...
        done_action = {
            "action": "done_ticket",
            "command": f"karya done {ticket_id}",
        ...
        block_action = {
            "action": "block_ticket",
            "command": f"karya block {ticket_id} \"<reason>\"",
        ...
        log_action = {
            "action": "log_ticket",
            "command": f"karya log {ticket_id} \"<message>\"",
    ...
    def exec(self, ticket_id: str | None = None) -> Dict[str, Any]:
        ...
        suggested_command = "karya log"
        for a in actions:
            if a["action"] == "start_ticket" and a.get("available"):
                suggested_command = f"karya start {ticket_id}"
                break
            if a["action"] == "done_ticket" and a.get("available"):
                suggested_command = f"karya done {ticket_id}"
                break
