...
from karya.core.models import ADR, ADRStatus, normalize_tag
...
class ADRService:
	...
	def create(
		...
		self._git.commit(f"karya: [{adr_id}] proposed ({actor or 'system'})")
		return adr
	...
	def update(self, adr_id: str, updates: dict, actor: str | None = None) -> ADR:
		...
		self._save(adr)
		self._git.commit(f"karya: [{adr_id}] updated ({actor or 'system'})")
		return adr
	...
	def accept(self, adr_id: str, actor: str | None = None) -> ADR:
		...
		self._save(adr)
		self._git.commit(f"karya: [{adr_id}] accepted ({actor or 'system'})")
		return adr
